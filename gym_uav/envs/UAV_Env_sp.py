'''
This is a UAV Env created for learning just the shortest path
No beam tracking or beam learning is involved here
'''
import gym
...
class UAV_Env_sp(gym.Env):
    def __init__(self):
        ...
        self.act_space = spaces.Discrete(len(self.ue_moves))  # n(RBD)*n(ue_moves)
    ...
    def step(self, action):
        ...
        ue_mv_ndx = self.decode_action(action)
        ...
        if (new_ue_xloc == 0) and (new_ue_yloc == 0):
            self.mimo_model = MIMO(np.array([40, 40, 0]), self.gNB[0], self.sc_xyz, self.ch_model, self.ptx, self.N_tx,
                                   self.N_rx)
    ...
    def _gameover(self):
        ...
        #    rwd = 0.6 * np.exp(-self.cur_dist / 1000) * np.exp(-2 * (self.steps_done - 1) / 50) * np.log10(
        #        8 * self.rate + 1)  # *np.exp(-2*(self.steps_done-1)/50)#*np.log10(self.rate + 1)#*np.exp(self.rate/20)#*min(np.exp(self.rate/20), np.exp((self.rate_threshold-self.rate)/20.0))#0.5 * np.exp(-1 * (self.steps_done - 1) / 50) *(1-self.rate/30)
            # print(rwd)
        #    done = False
